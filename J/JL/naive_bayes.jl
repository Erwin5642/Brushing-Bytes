...
data = readdlm("/home/joao/Brushing-Bytes/J/JL/playweather.csv", ','; skipstart = 1)
...
# previsao 1: newX = ["sunny", "hot"]

p_yes_newX = (len_sunny_yes / len_yes) * (len_hot_yes / len_yes) * p_yes 

p_no_newX = (len_sunny_no / len_no) * (len_hot_no / len_no) * p_no 

p_yes_newX_n = p_yes_newX / (p_no_newX + p_yes_newX)
p_no_newX_n = p_no_newX / (p_no_newX + p_yes_newX)

# previsao 2: newX = ["sunny", "cool", "high", "true"]
...
# P(A|B) = P(B|A) * P(A) / P(B)
# P(yes|newX) = P(newX|yes) * P(yes) / P(newX) = P(newX e yes) / P(newX)